...
import { X, Sparkles, Send, CheckCircle2, User, Phone, Mail, FileText, ChevronDown, Loader2 } from "lucide-react";
...
export default function BookingForm({ isOpen, onClose }: BookingFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Single call — Worker sends operator notification (Web3Forms) AND
      // client confirmation (Resend) from this one request.
      const res = await fetch("/api/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          form_type: "booking",
          subject: `Showroom Consultation Request — ${formData.name} — Italgres`,
      ...
      setSubmitResult("Your consultation request has been received. We will be in touch within one business day.");
    } catch (err: any) {
              ...
              {!submitResult ? (
                <form onSubmit={handleSubmit} className="space-y-5">
                  <p className="text-xs text-neutral-500 font-light leading-relaxed mb-4">
                    Register your private, 1-on-1 viewing appointment. Experience full-scale slabs on cantilever displays and map layouts under museum lights with our Showroom Director.
                  </p>
                  ...
                  {/* Notes */}
                  <textarea
                    placeholder="Briefly tell us about your design style or material look (optional)..."
                  ...
                  <div className="p-4 bg-[#FAF9F6] border border-neutral-200 rounded w-full max-w-sm text-left flex items-start space-x-3.5 shadow-sm">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-neutral-500 font-light leading-relaxed">
                        We will personally review your project specifications and send a text/email confirmation within 2 hours.
                      </p>
                    </div>
                  </div>
