...
export default function TourOverlay() {
  ...
  const activateStep = useCallback(async (s: number) => {
    ...
    if (rect) {
      ...
      setSpotlight(null);
      setTooltipStyle({ position: "fixed", bottom: 28, left: "50%", transform: "translateX(-50%)", width: 340, zIndex: 99 });
    }
  }, [positionTooltip]);
  ...
  return (
    <>
        ...
        {showWelcome && (
          <>
            ...
            <motion.div
              ...
              style={{ top: 0, left: 0, right: 0, bottom: 0, margin: "auto", width: 360, height: "fit-content", background: "#1C1A17", borderTop: "4px solid #f39b34" }}
            >
              <div className="p-8">
                <div className="w-12 h-12 flex items-center justify-center mb-5" style={{ background: "rgba(243,155,52,0.12)" }}>
                  <span style={{ fontSize: 24 }}>🪨</span>
                </div>
                <h2 className="text-lg font-bold mb-2" style={{ color: "#ffffff" }}>Welcome to Italgres Orlando</h2>
                <p className="text-sm leading-relaxed mb-6" style={{ color: "rgba(255,255,255,0.55)" }}>
                  Your private European tile catalog. Would you like a quick interactive tour?
                </p>
                <div className="flex gap-3">
                  <button onClick={() => { setShowWelcome(false); localStorage.setItem(STORAGE_KEY, "1"); }}
                  ...
                  <button onClick={() => { setShowWelcome(false); setStep(0); setActive(true); }}
      ...
      <AnimatePresence>
        {active && (
          <>
            {/* Spotlight — the highlighted element cuts through the overlay via box-shadow */}
            {spotlight && (
              <motion.div
                key={`spot-${step}`}
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                style={{
                  ...
                  // This creates the dark overlay everywhere EXCEPT this element
            ...
            >
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-white/10">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={finish} className="cursor-pointer" style={{ color: "rgba(255,255,255,0.3)" }}>
                    <X size={14} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between px-4 pb-4">
                <div className="flex items-center gap-1.5">
                  {STEPS.map((_, i) => (
                    <div key={i} style={{ width: i === step ? 18 : 5, height: 5, borderRadius: 999, background: i === step ? "#f39b34" : "rgba(255,255,255,0.15)", transition: "all 0.3s" }} />
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={finish} className="text-[10px] font-mono uppercase tracking-wider cursor-pointer mr-1" style={{ color: "rgba(255,255,255,0.25)" }}>
                    Skip
                  </button>
                  {step > 0 && (
                    <button onClick={prev} className="flex items-center gap-1 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider cursor-pointer" style={{ background: "rgba(255,255,255,0.08)", color: "#ffffff" }}>
                      <ChevronLeft size={11} /> Back
                    </button>
                  )}
                  <button onClick={next} className="flex items-center gap-1 px-4 py-1.5 text-[11px] font-bold uppercase tracking-wider cursor-pointer" style={{ background: "#f39b34", color: "#000" }}>
